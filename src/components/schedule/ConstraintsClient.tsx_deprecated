...
// ConstraintsClient — step 3 of schedule wizard.
// Receives data as props from server (no useEffect DB fetching).
// Manages: add/remove constraints, generate schedule.
...
export function ConstraintsClient({ scheduleId, locale, employees, shiftDefs, initialConstraints }: Props) {
  ...
  const CONSTRAINT_LABELS: Record<string, {
    ...
    needsValue?: string; needsTarget?: boolean; needsShift?: boolean
  }> = {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      router.push(`/${locale}/schedules/${scheduleId}`)
    } catch {
  ...
  function getConstraintSummary(c: Constraint): string {
    ...
    const parts  = [labelMeta?.label ?? c.type]
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex gap-2 mb-8">
        {[1, 2, 3].map(s => (
          <div key={s} className="h-1.5 flex-1 rounded-full" style={{ background: '#2563eb' }} />
        ))}
      </div>
      ...
      {/* Add constraint form */}
      {showForm && (
        <div className="bg-white rounded-xl p-5 mb-6" style={{ border: '0.5px solid #bfdbfe' }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium mb-1" style={{ color: '#6b7280' }}>
                {tCon('employee')} ({tCommon('optional')})
              </label>
            ...
            {meta?.needsValue && (
              <div>
                ...
                <input {...register('value', { valueAsNumber: true })} type="number" min={1}
                  className="w-full px-3 py-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  style={{ border: '0.5px solid #d1d5db', color: '#111827', background: '#fff' }} />
              </div>
            )}
