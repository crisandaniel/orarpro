...
// New schedule wizard — step 1.
// Reads org_type from ?org= query param via useSearchParams (SSR-safe).
// Locale from useParams — no window.location.
...
const DAYS = [
  ...
  { value: 4, label: 'Joi' }, { value: 5, label: 'Vin' }, { value: 6, label: 'Sâm' },
  { value: 7, label: 'Dum' },
]
...
export default function NewSchedulePage() {
  ...
  const orgType  = (searchParams.get('org') ?? 'business') as 'business' | 'education'
  const defaultType = orgType === 'education' ? 'school' : 'shifts'
  ...
  // Keep form in sync with orgType (handles hydration)
  useEffect(() => {
  ...
  const workingDays     = watch('workingDays') ?? [1, 2, 3, 4, 5]
  ...
  const typeOptions = [
    { value: 'shifts', icon: Factory,       title: 'Ture / Schimburi', desc: 'HoReCa, fabrici, retail, clinici',          hidden: orgType === 'education' },
    { value: 'school', icon: GraduationCap, title: 'Orar școlar',      desc: 'Școli, licee, grădinițe, universități', hidden: orgType === 'business'  },
  ].filter(o => !o.hidden)
  ...
  async function onSubmit(data: Step1Form) {
    ...
    if (!res) { toast.error(tCommon('error')); return }

    const result = await res.json()
    if (!res.ok) { toast.error(result.error ?? tCommon('error')); return }

    const nextStep = data.type === 'school'
      ? `/${locale}/schedules/${result.scheduleId}/school-setup`
      : `/${locale}/schedules/${result.scheduleId}/setup`
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Type selector — only if org has multiple options */}
        {typeOptions.length > 1 && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {typeOptions.map(({ value, icon: Icon, title, desc }) => (
                <label key={value}
        ...
        <div>
          ...
          <input {...register('name')}
            ...
            placeholder={scheduleType === 'school' ? 'Semestrul 1 2025-2026' : 'Săptămâna 23 — Bucătărie'}
          />
        ...
        <div className="grid grid-cols-2 gap-4">
          <DatePicker label={t('startDate')} value={watch('startDate') ?? ''}
            onChange={d => setValue('startDate', d)} placeholder={t('selectStartDate')}
            error={errors.startDate?.message} />
          <DatePicker label={t('endDate')} value={watch('endDate') ?? ''}
            onChange={d => setValue('endDate', d)} placeholder={t('selectEndDate')}
            minDate={watch('startDate')} error={errors.endDate?.message} />
        </div>
          ...
          <div className="flex gap-2">
            {DAYS.map(({ value, label }) => (
              <button key={value} type="button" onClick={() => toggleDay(value)}
                ...
                style={{
                  background: workingDays.includes(value) ? '#2563eb' : '#f3f4f6',
                  color:      workingDays.includes(value) ? '#fff'    : '#6b7280',
                }}>
                {label}
              </button>
            ))}
        ...
        <button type="submit" disabled={isSubmitting}
          className="w-full flex items-center justify-center gap-2 py-3 px-4 text-white rounded-xl text-sm font-medium disabled:opacity-50"
